...
export default function SubjectManagementPage() {
  ...
  // Fetch initial data (combined, streams, degrees, years)
  useEffect(() => {
    const getData = async () => {
      try {
        const [combinedRes, streamRes, degreeRes, yearRes] = await Promise.all([
        ...
        if (combinedRes.ok && streamRes.ok && degreeRes.ok && yearRes.ok) {
          const [combinedData, streamData, degreeData, yearData] = await Promise.all([
            combinedRes.json(),
            streamRes.json(),
            degreeRes.json(),
            yearRes.json(),
  ...
  const exportToExcel = () => {
    const dataToExport = filteredSubjects.map(subject => ({
      ...
      'Exam Date': formatDate(new Date(subject.exam)),
      'Course': courses.find(c => c.uuid === subject.course)?.name || '',
      'Combined': combined.find(c => c.uuid === subject.combined)?.name || '',
      'Status': subject.isActive === false ? 'Deactivated' : 'Active'
    }));
  ...
  const handleImportExcel = async (event) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const subjectsToImport = jsonData.map(item => ({
          name: item['Subject Name'],
          code: item['Code'],
          type: item['Type'],
          semester: item['Semester'],
          exam: new Date(item['Exam Date']).toISOString(),
          course: courses.find(c => c.name === item['Course'])?.uuid || '',
          combined: combined.find(c => c.name === item['Combined'])?.uuid || '',
          uuid: [...Array(6)].map(() => Math.random().toString(36)[2].toUpperCase()).join(""),
        }));

        // Validate required fields
        const invalidSubjects = subjectsToImport.filter(sub => 
          !sub.name || !sub.code || !sub.type || !sub.semester || !sub.exam || !sub.course || !sub.combined
        );

        if (invalidSubjects.length > 0) {
          toast.error(`${invalidSubjects.length} records have missing required fields`);
          return;
        }

        // Send to API
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/subjects/bulk`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (res.ok) {
          const data = await res.json();
          setSubjects(prev => [...prev, ...data]);
          toast.success(`${data.length} subjects imported successfully!`);
  ...
  const handleToggleSubjectStatus = async (uuid) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/subjects/${uuid}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-48">
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem onSelect={exportToExcel} className="gap-2">
                  <FileDown className="h-4 w-4" />
                  Export
                </DropdownMenuItem>
