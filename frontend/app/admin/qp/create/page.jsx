...
export default function ImportQuestionPaperPage() {
  ...
  useEffect(() => {
    if (selectedSubject) {
      setExamName(`${selectedSubject.name} Exam`);
    }
  }, [selectedSubject]);
  ...
  const handleFileUpload = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/qp/upload`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-4 bg-white p-4 sm:p-6 rounded-xl shadow-sm">
            ...
            >
              ...
              <span className="text-sm sm:text-base">Complete Import</span>
            </div>
          </div>
        </header>
                            ...
                            {isLoading ? (
                              ...
                              courses
                                .filter(
                                  (course) =>
                                    !selectedCombineds ||
                                    selectedCombineds.course.includes(
                                      course.uuid
                                    )
                                )
                                .map((course) => (
                        ...
                        <DropdownMenuContent className="w-full min-w-[200px]">
                          <DropdownMenuGroup>
                            {isLoading ? (
                              ...
                              Array.from(
                                { length: selectedCourse.semCount },
                                (_, i) => i + 1
                              ).map((semesterNum) => (
                                <DropdownMenuItem
                                  key={semesterNum}
          ...
          {step === 2 && (
            <Card className="mb-8">
              ...
              <CardFooter className="flex justify-end gap-4 border-t">
                ...
                <Button className="cursor-pointer" onClick={() => router.push('/admin/qp/master')}>Complete Import</Button>
              </CardFooter>
            </Card>
          )}
        </div>
